...
pima_labels = []
min_max = [[i, i] for i in pima_data[0]]
del min_max[-1]
for pima in pima_data:
    pima_labels.append(pima[-1])
    for idx in range(len(min_max)):
        if pima[idx] != 'na':
            if pima[idx] < min_max[idx][0]:
                min_max[idx][0] = pima[idx]
            if pima[idx] > min_max[idx][1]:
                min_max[idx][1] = pima[idx]
...
for pima in pima_data:
    ...
    for idx in range(len(min_max)):
        ...
        normed_pima.append((pima[idx] - min_max[idx][0]) / val_range)
    norm_pima.append(normed_pima)

print("normalised pima")

from copy import deepcopy
training_set_pimas = deepcopy(norm_pima)
