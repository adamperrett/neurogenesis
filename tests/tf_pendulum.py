...
for trial in range(repeats):
    ...
    for attempt in range(max_attemps_until_fail):  # Run until solved/fail
        state = env.reset()
        episode_reward = 0
        with tf.GradientTape() as tape:
            ...
            balance_lengths.append(episode_reward)
        ...
        if episode_count % 10 == 0:
            template = "running reward: {:.2f} - average reward: {:.2f} at episode {} of trial {}"
            print(template.format(running_reward, average_reward, episode_count, trial))

        if len(balance_lengths) > 100 and average_reward > 475:
            print(test_label)
            print("Solved at episode {}!".format(episode_count))
            print("Running reward:", running_reward, "- Average reward:", average_reward)
            solve_lengths.append(episode_count)
            # learning_history.append(balance_lengths)
            break
        # if running_reward > 475:  # Condition to consider the task solved
        #     print("Solved at episode {}!".format(episode_count))
        #     print("Running reward:", running_reward, "- Average reward:", np.average(balance_lengths[-100:]))
        #     break
    if attempt >= max_attemps_until_fail-1:
